  ...
  if (!products.length) {
    ...
    grid.innerHTML = products.map(p => {
      const media = p.image_url
        ? '<img class="product-photo" src="' + escapeHTML(p.image_url) + '" alt="' + escapeHTML(p.name) + '" loading="lazy" decoding="async"><span class="image-sheen" aria-hidden="true"></span>'
        : '<div class="product-placeholder" aria-label="No product image available"><span>A</span></div>';
      return '<article class="product"><div class="product-img">' + media + '<span>' + escapeHTML(p.tag) + '</span></div><div class="product-info"><div><h3>' + escapeHTML(p.name) + '</h3><p class="product-description">' + escapeHTML(p.description) + ' · <b>' + escapeHTML(p.price) + '</b></p></div><a class="enquire" data-product="' + p.id + '" href="#">Enquire ↗</a></div></article>';
    }).join('');
